\documentclass[12pt]{article}
\usepackage[a4paper,margin=0.75in]{geometry}
\usepackage{amssymb} % For checkboxes

\setlength{\parindent}{0pt} % Disable paragraph indentation

\begin{document}

\noindent \textbf{Form No. OGC-SF-2006-02} % Bold at top left

\begin{center}
    {\fontsize{11pt}{13pt} \selectfont \textbf{AUTHORIZATION TO RELEASE EDUCATIONAL RECORDS}}\\
    {\fontsize{11pt}{13pt} \selectfont \textbf{Family Educational Rights and Privacy Act of 1974 as Amended (FERPA)}}
\end{center}

I \underline{\hspace{9cm}} hereby voluntarily authorize officials in the\\
\hspace{2cm} [Print Name of Student] \\

University of Houston - \underline{\hspace{7cm}} identified below to disclose personally identifiable information from my educational records.

\textbf{(Please check the box or boxes that apply):}
\begin{list}{}{\leftmargin=0pt \itemindent=0pt \setlength{\itemsep}{0pt}}
    \item $\square$ Office of the University Registrar
    \item $\square$ Scholarships and Financial Aid
    \item $\square$ Student Financial Services
    \item $\square$ Undergraduate Scholars @ UH (formally USD)
    \item $\square$ University Advancement
    \item $\square$ Dean of Students Office
    \item $\square$ Other (Please Specify) \underline{\hspace{5cm}}
\end{list}

\textbf{Specifically, I authorize disclosure of the following information or category of information.}\\
\textbf{(Please check the box or boxes that apply):}
\begin{list}{}{\leftmargin=0pt \itemindent=0pt \setlength{\itemsep}{0pt}}
    \item $\square$ Academic Advising Profile/Information
    \item $\square$ Academic Records
    \item $\square$ All University Records
    \item $\square$ Billing/Financial Aid
    \item $\square$ Disciplinary
    \item $\square$ Grades/Transcripts
    \item $\square$ Housing
    \item $\square$ Photos
    \item $\square$ Scholarship and/or Honors
    \item $\square$ Other (Please Specify) \underline{\hspace{5cm}}
\end{list}

\textbf{This information may be released to:} \underline{\hspace{9cm}}\\
[Print Name(s) of Individual(s) To Whom University May Disclose Information]

\underline{\hspace{9cm}} for the purpose of informing:\\
[List Additional Individuals if Necessary]

\textbf{(Please check the box or boxes that apply):}
\begin{list}{}{\leftmargin=0pt \itemindent=0pt \setlength{\itemsep}{0pt}}
    \item $\square$ Family
    \item $\square$ Educational Institution
    \item $\square$ Honor or Award
    \item $\square$ Employer/Prospective Employer
    \item $\square$ Public or Media of Scholarship
    \item $\square$ Other (Please Specify) \underline{\hspace{5cm}}
\end{list}

\textbf{Please provide a password to obtain information via the phone:} \underline{\hspace{3.5cm}}\\
\footnotesize (Password should not contain more than ten (10) letters. The University will not release information to the caller if they do not have the password. A new form must be completed to change your password.)

\vspace{0.3cm}
\textbf{This is to attest that I am the student signing this form.} I understand the information may be released orally or in the form of copies of written records, as preferred by the requester. This authorization will remain in effect from the date it is executed until revoked by me, in writing, and delivered to Department(s) identified above.

\vspace{0.6cm}
\underline{\hspace{5.5cm}}\\
PeopleSoft I.D. Number

\vspace{0.3cm}
\underline{\hspace{5.5cm}} \hspace{0.5cm} \underline{\hspace{5.5cm}}\\
Student Name [please print] \hspace{2cm} Student Signature

\vspace{0.6cm}
\textbf{Please Retain a Copy for your Records}\\
\textbf{Document may be Submitted to Registrar’s Office}\\
FERPA Authorization Form OGC-SF-2006-02\\
Revised 11.10.2022\\
Page 1 of 1

\vfill
\hfill \textbf{Note: Modification of this Form requires approval of OGC}

\end{document}
